...
df['city_tv'] = df['city_tv'].astype(float)
...
y = df['city_tv']  # 目标变量（这里以CSM52城市网收视率为例）
...
plt.xlabel('集数')
plt.ylabel('收视率(%)')
plt.title('2025年类似《人民的名义》电视剧收视率预测值和真实值对比')
plt.legend()
plt.savefig("2025年类似《人民的名义》电视剧收视率预测值和真实值对比")
plt.show()
...
# 可视化呈现2025年预测结果（预测收视率随集数变化趋势）
plt.plot(future_df['num'], future_predictions)
plt.xlabel('剧集')
plt.ylabel('收视率(%)')
plt.title('2025年类似《人民的名义》电视剧收视率预测')
plt.savefig("2025年类似《人民的名义》电视剧收视率预测")
plt.show()